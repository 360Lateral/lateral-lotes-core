...
export default function SeccionGeotecnico({ loteId, pdfProps, qk: qkProp, onSaved }: SeccionProps & { lat?: number | null; lng?: number | null }) {
  const qk = qkProp ?? ["analisis-geotecnico", loteId];
  const { data } = useQuery({
    queryKey: qk,
    queryFn: async () => {
  ...
  const upsert = useAnalisisUpsert("analisis_geotecnico", loteId, qk, onSaved);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <PdfExtractToggle areaKey="geotecnico" pdfProps={pdfProps} />
      </div>
