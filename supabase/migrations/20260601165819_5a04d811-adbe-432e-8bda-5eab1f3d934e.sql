...
BEGIN
  ...
  -- Solo cuenta como firmado si firmó la versión vigente del Acuerdo de No Elusión
  IF v_uid IS NULL THEN
    v_tiene_nda := false;
  ELSE
    SELECT EXISTS (
  ...
  IF v_nivel = 'premium' AND v_tiene_nda THEN
    v_resultado := v_resultado || jsonb_build_object(
      'precio_venta_estimado', v_lote.precio_venta_estimado,
      'notas', v_lote.notas,
      'tiene_analisis_juridico', v_lote.tiene_analisis_juridico,
      'tiene_analisis_ambiental', v_lote.tiene_analisis_ambiental,
      'tiene_analisis_arquitectonico', v_lote.tiene_analisis_arquitectonico,
      'tiene_analisis_financiero', v_lote.tiene_analisis_financiero,
      'tiene_analisis_geotecnico', v_lote.tiene_analisis_geotecnico,
      'tiene_analisis_mercado', v_lote.tiene_analisis_mercado,
      'tiene_analisis_sspp', v_lote.tiene_analisis_sspp
    );
  END IF;
