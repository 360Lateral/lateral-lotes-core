...
CREATE OR REPLACE FUNCTION public.firmar_nda(
  p_lote_id uuid,
  p_version_nda text,
  p_contenido_aceptado text,
  p_user_agent text
)
RETURNS uuid
LANGUAGE plpgsql
...
BEGIN
  v_uid := auth.uid();
  IF v_uid IS NULL THEN
    RAISE EXCEPTION 'Debes estar autenticado para firmar el NDA';
  END IF;
  ...
  IF NOT v_es_dev THEN
    RAISE EXCEPTION 'Solo desarrolladores pueden firmar NDAs de lotes';
  END IF;
  ...
  SELECT id INTO v_firma_id
   ...
   WHERE desarrollador_id = v_uid
     AND lote_id = p_lote_id
     AND version_nda = p_version_nda;

  IF v_firma_id IS NOT NULL THEN
    RETURN v_firma_id;
  END IF;

  INSERT INTO public.ndas_firmados (
    desarrollador_id, lote_id, version_nda, contenido_aceptado,
    ip, user_agent
  ) VALUES (
    v_uid, p_lote_id, p_version_nda, p_contenido_aceptado,
...
GRANT EXECUTE ON FUNCTION public.firmar_nda(uuid, text, text, text) TO authenticated;