...
BEGIN
  ...
  v_es_propietario := (v_uid IS NOT NULL AND (v_lote.propietario_id = v_uid OR v_lote.owner_id = v_uid));
  ...
  IF NOT v_es_propietario AND NOT v_es_admin THEN
    IF v_lote.publicado_venta IS NOT TRUE OR v_lote.estado_publicacion <> 'aprobado' THEN
      IF NOT COALESCE(v_lote.es_ejemplo, false) THEN
        RETURN jsonb_build_object('error', 'Lote no disponible públicamente');
      END IF;
    END IF;

    IF NOT public.tiene_acceso_ficha(v_uid, p_lote_id) THEN
  ...
  IF v_nivel IN ('profesional','premium') AND (v_tiene_nda OR v_tiene_ppv) THEN
    v_resultado := v_resultado || jsonb_build_object(
      'direccion', v_lote.direccion,
      'matricula', v_lote.matricula_inmobiliaria,
      'lat', v_lote.lat,
