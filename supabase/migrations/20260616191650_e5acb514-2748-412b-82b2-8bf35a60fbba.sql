...
AS $$
  SELECT EXISTS (
    ...
    WHERE l.id = _lote_id
      AND (
        l.owner_id = _user_id
        OR l.propietario_id = _user_id
        OR public.is_admin_or_experto(_user_id)
        OR EXISTS (
          SELECT 1
          FROM public.usuario_owner uo
          WHERE uo.user_id = _user_id
            AND uo.owner_id IN (l.owner_id, l.propietario_id)
        )
        OR EXISTS (
          SELECT 1
        ...
        OR EXISTS (
          SELECT 1
          FROM public.ndas_firmados n
          WHERE n.lote_id = _lote_id
            AND n.desarrollador_id = _user_id
        )
      )
  )
$$;
...
USING (
  ...
  OR public.user_shares_owner_org(auth.uid(), propietario_id)
  OR public.user_has_active_engagement_on_lote(auth.uid(), id)
