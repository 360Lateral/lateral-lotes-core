...
CREATE POLICY "Dev ve sus matches" ON public.criteria_matches
  ...
  USING (
    alerta_id IN (SELECT id FROM public.alertas WHERE user_id = auth.uid())
  );
